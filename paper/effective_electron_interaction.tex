...
The renormalized parameters are defined through physical observables in terms of one-particle and two-particle vertex functions. The one-particle vertex function, namely the self-energy, can be derived from the fully dressed propagator of the electron,
\begin{equation}
    G_k\equiv \left< \mathcal{T} \hat{c}(\tau_1 \mathbf{x}_1) \hat{c}^\dag(\tau_2 \mathbf{x}_2) \right>_k,
\end{equation}
so that the self-energy of the bare electron,
\begin{equation}
    \Sigma_k = G_k^{-1}-g_k^{-1}.
\end{equation}

According to the Landau Fermi liquid theory, the renormalized parameters $z$, $m^*$ and $\mu_R$ are fixed by the ow-energy limit of the self-energy near the Fermi surface defines. We will simply quote the Fermi liquid results in literature,

\begin{enumerate}
    \item The renormalized chemical potential is defined as,
          \begin{equation}
              \label{eq:condition1}
              \mu_R \equiv \mu- \operatorname{Re}\Sigma_{(0, k_F)}
          \end{equation}

          At a sufficiently low temperature, the renormalized chemical potential potential $\mu_R$ should be the Fermi energy $E_F$, which is an input parameter of the problem.

    \item The wave-function renormalization factor $z$ is fixed by the small frequency behavior of the self-energy near the Fermi surface,
          \begin{equation}
              \label{eq:condition2}
              \frac{1}{z} = 1-\frac{\partial \operatorname{Im} \Sigma_{(0, k_F)}}{\partial i\omega},
          \end{equation}
    \item The effective mass is fixed by both the small momentum and the frequency behavior of the self-energy near the Fermi surface,
          \begin{equation}
              \label{eq:condition3}
              \frac{m}{m^*}=z \cdot \left(1+\frac{m}{k_F}\frac{\partial \operatorname{Re}\Sigma_{(0, k_F)}}{\partial k}\right).
          \end{equation}
\end{enumerate}

The remaining renormalized parameter $f^+$ is the Landau parameter of the quasiparticle interaction. It can extracted from the two-electron scattering amplitude. More specifically,  from the connected two-body Green's function,
\begin{equation}
    G^c_{k_1k_2; k_3k_4}\equiv \left< \mathcal{T} \hat{c}(\tau_1 \mathbf{x}_1)\hat{c}(\tau_2 \mathbf{x}_2) \hat{c}^\dag(\tau_3 \mathbf{x}_3)\hat{c}^\dag(\tau_4 \mathbf{x}_4) \right>^c_{k_1 k_2 k_3 k_3},
\end{equation}
one can derive the $4$-point 1PI vertex function, or the scattering amplitude, by amputating the two-body Green's function with the one-electron propagator,
\begin{equation}
    \Gamma^4_{k_1k_2;k_3k_4}\equiv G^c_{k_1k2; k3k4}/\left[G(k_1)G(k_2)G(k_3)G(k_4)\right].
\end{equation}

The parameter $f^+$ is fixed to the spin-symmetric Landau parameter with $l=0$,
\begin{equation}
    \label{eq:condition4}
    f^+ = z^2 \left. \overline{\Gamma_4(k_1, k_2; k_1-q, k_2+q)}\right|^+_{q\rightarrow 0, q_0 \gg v_F^*|\bq| }-v_q.
\end{equation}
where the symbol $\overline{\Gamma_4}$ means projecting the incoming momentum-frequency to the Fermi surface $k_1=(0, k_F\mathbf{n}_1)$ and $k_2=(0, k_F\mathbf{n}_2)$, then average over the orientation of the unit vectors $\mathbf{n}_1$ and $\mathbf{n}_2$.

The equations Eq. \eqref{eq:condition1},\eqref{eq:condition2}, \eqref{eq:condition3} and \eqref{eq:condition4} are called renormalization conditions.
...
\begin{align*}
    \delta_R & =  z^2\left(\frac{R_q\xi+f^+}{1+(R_q\xi+f^+)\Pi_0^*}-f^+\right)\xi - R_q\xi \\
             & =           \{2z_1 R_q-[(R_q+f_1)^2+f_2]\Pi_0^*\}\xi^2+...
\end{align*}
...
The building blocks of the diagrams are given in Fig.\eqref{fig:component}.
