...
_green = Dict()
for p in partition
    d = Parquet.green(diagPara(p[1])).diagram
...
_green = [_green[p] for p in partition]
const diagpara = [diags[1].id.para for diags in _green]
const diag = [ExprTree.build(diags) for diags in _green]
...
function integrand(config)
    ...
    w = diag[order].node.current[root[order]]
...
function MC()
    ...
    dof = [[p.innerLoopNum, p.totalTauNum - 2] for p in diagpara] # K, T, ExtKidx
    # dof = [[1, 1, 1], [1, 1, 1], [1, 1, 1], [2, 3, 1]]
    # dof = [[1, 0, 1], [1, 0, 1], [1, 0, 1], [2, 1, 1]]
    # println(dof)
    # exit(0)
    obs = zeros(Float64, length(dof), Nl) # observable for the Fock diagram 
