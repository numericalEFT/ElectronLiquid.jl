...
function DiagramdkWeight(pidx, vars, config)
    ...
    for (i, lfstat) in enumerate(leafstates[idx])
        ...
        elseif lftype == 2 #bosonic
            ...
            if dim == 3
                ...
                if dk_order != 0
                    leafval[idx][i] *= -2(order + 1) * invK * kq[1] * momLoopPool[leafMomIdx][1]
                end
            elseif dim == 2
                ...
                if isLayered2D == false
                    ...
                    if dk_order != 0
                        leafval[idx][i] *= -(order + 1) * invK * kq[1] / abskq * momLoopPool[leafMomIdx][1]
                    end
                else
                    if order == 0
                        q = sqrt(abskq + 1e-16)
    ...
    group = partition[idx]
    evalfuncParquetAD_sigmadk_map[group](root, leafval[idx])
...
function measuredk_Reweight(pidx, vars, obs, relative_weight, config) # for the mcmc algorithm
    varK, varT, varN, ExtKidx = vars
    para, kgrid, ngrid, MaxLoopNum, extT_labels = config.userdata[1:5]
    leafstates, leafval = config.userdata[6], config.userdata[7]
    momLoopPool, root = config.userdata[8:9]
    ...
    weight = DiagramdkWeight(pidx, vars, config)
    loopNum = config.dof[pidx][1]
    factor = 1.0 / (2π)^(dim * loopNum)
    ...
    for (j,iidx) in enumerate(part_list[pidx])
        for (i, lfstat) in enumerate(leafstates[iidx])
            ...
            elseif lftype == 2 #bosonic
                ...
                if dim == 3
                    ...
                    if dk_order != 0
                        leafval[iidx][i] *= -2(order + 1) * invK * kq[1] * momLoopPool[leafMomIdx][1]
                    end
                elseif dim == 2
                    abskq = sqrt(dot(kq, kq))
                    if isLayered2D == false
                        ...
                        if dk_order != 0
                            leafval[iidx][i] *= -(order + 1) * invK * kq[1] / abskq * momLoopPool[leafMomIdx][1]
                        end
                    else
                        if order == 0
                            q = sqrt(abskq + 1e-16)
        ...
        evalfuncParquetAD_sigmadk_map[group](root, leafval[iidx])
        n = ngrid[nidx]
...
function ParquetAD_dk_Reweight(para::ParaMC, diagram_info;
    kgrid=[para.kF,],
    ngrid=[0,],
    ...
    result = integrate(integrand; config=config, measure=measuredk_Reweight, thermal_ratio=0.2, print=print, neval=neval, solver=solver, kwargs...)
...
function MC_dk_Reweight(para; kgrid=[para.kF,], ngrid=[0], neval=1e6, reweight_goal=nothing,
