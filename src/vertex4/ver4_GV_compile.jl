function compileC_GV_toFiles(order, partition, FeynGraphs, maxloopNum::Int; datatype::DataType=Float64,
    root_dir=joinpath(@__DIR__, "source_codeGV"), c_source=joinpath(root_dir, "func_O$(order)_ver4GV.c"),
    lib_path=root_dir, lib_name="ver4O$(order)GV", compiler::String="gcc", isnative::Bool=false)

    ### compile the Parquet + Taylor-AD generated Graphs to C language source code
    println("compiling to c code")
    leaf_maps = GVcompileC_toFile(order, partition, FeynGraphs, root_dir, c_source=c_source)
    println("compiled")

    # ### compile the C language 
    println("compiling c code to .so lib")
    GVcompileC_so(order, partition, datatype; c_source=c_source,
        lib_path=lib_path, lib_name=lib_name, compiler=compiler, isnative=isnative)
    println("compiled")
    GV_Cwrapper(order, partition, datatype, lib_path=lib_path, lib_name=lib_name)
...
function GVcompileC_toFile(order, partition, FeynGraphs,
    root_dir=joinpath(@__DIR__, "source_codeGV"); c_source=joinpath(root_dir, "func_O$(order)_ver4GV.c"), datatype::DataType=Float64)
...
function extT_and_spin_toFile(order, partition, FeynGraphs, root_dir=joinpath(@__DIR__, "source_codeGV"))
    jldopen(joinpath(root_dir, "extT_spin_O$(order)_GV.jld2"), "w") do f
        for key in partition
            key_str = join(string.(key))
...
function GVcompileC_so(order, partition, datatype::DataType=Float64; c_source=joinpath(@__DIR__, "source_codeGV", "func_O$(order)_ver4GV.c"),
    lib_path=joinpath(@__DIR__, "source_codeGV"), lib_name="ver4O$(order)GV", compiler::String="gcc", isnative::Bool=false)
...
function GV_Cwrapper(order, partition, datatype::DataType=Float64; lib_path=joinpath(@__DIR__, "source_codeGV"), lib_name="ver4O$(order)GV")
...
function leafinfo_toFile(order, partition, leaf_maps::Vector{Dict{Int,Graph}}, maxloopNum::Int, root_dir=joinpath(@__DIR__, "source_codeGV/"))
    ...
    CSV.write(joinpath(root_dir, "loopBasis_GVmaxOrder$order.csv"), df)
end
