function integrand_lavg_Project(idx, var, config)
...
function measure_lavg_Project(idx, var, obs, weight, config)
    ...
    for (Lidx, l) in enumerate(lgrid)
        ...
        lfactor = legendfactor(x, l, dim)
        obs[idx][1, Lidx, Kidx] += weight.d * lfactor
        obs[idx][2, Lidx, Kidx] += weight.e * lfactor
    end
end

function lavg_Project(para::ParaMC, diagram;
    chan=:PH,  # or :PP
    ...
    result = integrate(integrand; measure=measure_lavg_Project, config=config, solver=solver, neval=neval, print=print, kwargs...)
...
function MC_lavg_Project(para; kamp=[para.kF,], kamp2=kamp, q=[0.0 for k in kamp], n=[-1, 0, 0, -1], l=[0,],
    ...
    ver4, result = Ver4.lavg_Project(para, diaginfo; chan = channel,
