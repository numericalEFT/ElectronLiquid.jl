function integrand_AR_Clib(idx, var, config)
    para, chan, filter, kampgrid, kamp2grid, qgrid, Anggrid, n = config.userdata[1:8]
...
function measure_AR(idx, var, obs, weight, config)
    Thidx = var[3][1]
    Kidx = var[4][1]
    obs[idx][1, Thidx, Kidx] += weight.d
    obs[idx][2, Thidx, Kidx] += weight.e
end


function AR_Clib(para::ParaMC, diagram;
    chan=:PH,  # or :PP
    ...
    config=nothing,
    solver=:mcmc,
    integrand::Function=integrand_AR_Clib,
    ...
    obs = [zeros(ComplexF64, 2, NA, Nk) for _ in diagpara]
    ...
    if isnothing(config)
        config = MCIntegration.Configuration(;
            ...
            userdata=(para, chan,filter, kamp, kamp2, q, theta, n, maxMomNum, extT_labels,
                spin_conventions, leafstates, leafvalues, momLoopPool,
                root, partition),
            kwargs...
        )
    end
    result = integrate(integrand; measure=measure_AR, config=config, solver=solver, neval=neval, print=print, kwargs...)
...
function MC_AR_Clib(para; kamp=[para.kF,], kamp2=kamp, q=[0.0 for k in kamp], n=[-1, 0, 0, -1], theta = [0,],
    ...
    ver4, result = Ver4.AR_Clib(para, diaginfo;
        kamp=kamp, kamp2=kamp2, q=q, n=n, theta = theta,
        ...
        for p in partition
            ...
            for (ti, _theta) in enumerate(theta)
                ...
                for (ki, k) in enumerate(kamp)
                    factor = 1.0
                    d1, d2 = real(data[1, ti, ki]) * factor, real(data[2, ti, ki]) * factor
