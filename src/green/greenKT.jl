...
function KT(para::ParaMC, diagram;
    kgrid=[para.kF,],
    tgrid=[para.beta - 1e-8,], #must be (0, beta)
    ...
    T.data[1] = 0.0
    T.data[2] = tgrid[1]
    X = MCIntegration.Discrete(1, length(tgrid), alpha=alpha)
    ExtKidx = MCIntegration.Discrete(1, length(kgrid), alpha=alpha)

    dof = [[p.innerLoopNum, p.totalTauNum - 2, 1, 1] for p in diagpara] # K, T, ExtKidx
