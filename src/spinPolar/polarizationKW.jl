...
function integrandKW_Clib(idx, vars, config)
    ...
    group = partition[idx]
    # if Generator == :GV
    evalfuncGV_spinPolar_map[group](root, leafval)
    # else
    #     evalfuncParquetAD_spinPolar_map[group](root, leafval)
    # end 
...
function KW(para::ParaMC, diagram;
    ...
    result = integrate(integrand; config=config, measure=measureKW, print=print, neval=neval, solver=solver, thermal_ratio=0.2, kwargs...)
...
function KW_Clib(para::ParaMC, diagram_info;
    ...
    df = CSV.read(root_dir * "loopBasis_$(name)_maxOrder5.csv", DataFrame)
