...
# new version of mcpcf
# mc only do R = 1 - \Gamma F
# use DLR fourier to compute R -> F
...
function update!(result; α=α)
    ...
    # update to Ri, Ft
    calcF!(funcs.Ft, funcs.Ris, funcs.Rt, param; norm=(1 - α))
end
...
function run(steps, param, alg=:vegas)
    ...
    if isload
        ...
        Ri, Rt, Ft = Propagators.initR(param; mint=mint, minK=minK, maxK=maxK, order=order)
    end
